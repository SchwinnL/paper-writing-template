...
%\label{sec:pos-goals-definition}
Over the past decade, robustness research primarily aimed to define and improve single-output models $f\,$$:$$\, \gX \rightarrow \gY$ for classification ($\gY = \{1, \ldots, C\}$) or regression ($\gY=\sR$), and multi-output models $f : \gX \rightarrow \gY^d$. % (e.g., graph-structured data)
...
%\label{sec:pos-goals-definition-llm}
...
%\todo{Include discussion about diverse metrics, efficiency, perplexity, transferability, robustness-accuracy trade-off}

%\todo{Add sth about ASR not capturing everything in LLMs}
...
%\label{sec:pos-capability-definition}
%\subsubsection{Previous Capabilities}
As detailed in \S\ref{sec:tax-capability}, attacker \textbf{knowledge} of the model $f$ and the underlying defense mechanisms can range from black-box to white-box scenarios, each with varying levels of access. Past works generally recommended evaluating defenses in the white-box threat model due to multiple reasons~\cite{athalye_obfuscated_2018}. %, carlini_evaluating_2019, tramer_adaptive_2020 
...
%\textbf{Third-party evaluations.} Many defenses against adversarial attacks proposed in earlier research were later found to be ineffective. Numerous studies have demonstrated this by developing adaptive attacks that successfully bypassed over $50$ defenses published in top machine learning venues~\cite{uesato2018adversarial, tramer_adaptive_2020}. While we believe adaptive attacks remain crucial for reliable evaluations, we argue that a similar pattern is unlikely to emerge in LLM adversarial robustness research. The high computational cost of running these models, combined with the inefficiency of current attacks, makes breaking them significantly more time- and resource-intensive~\cite{andriushchenko2024jailbreaking}. Thus, many researchers will lack resources or incentives to conduct third-party evaluations.
...
%Numerous studies demonstrated this by developing adaptive attacks that bypassed over 50 defenses published in top machine learning venues~\cite{tramer_adaptive_2020}. 
...
\textbf{Academia's role.} %in adversarial alignment. 
...
%\footnote{The goal of this position paper is to identify misaligned objectives in \textit{academia}. Our intent is not to offer a political stance on the risks associated with LLMs nor to critique the role of industry.} 
